 ...
 * (options.tolerant, options.returnUnsorted) -- see patches #5 and #6 notes
 */
tokenize = Sizzle.tokenize = function( selector, parseOnly, options) {
	...
	if (validLen !== 0) {
		...
		// Avoid caching tokens when parsing an invalid result
		return {
			selector: validSelector,
			tokens: returnUnsorted ? groups : sortedGroups,
			nextIndex: nextIndex
		};
	}
