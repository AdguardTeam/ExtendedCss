 ...
 * 
 * Patch #6:
 * Changing "tokenize" method farther. The result of the tokenize is re-sorted
 * in order to defeat the issues caused by slow pseudo-classes while matching right-to-left.
 */
 ...
 * 
 * @param {*} tokens a list of tokens to sort
 */
var TokenSorter = (function() {
	...
	/**
	 * Does all the sorting work
	 * 
	 * @param {*} tokens a list of tokens to sort
	 * @returns a re-sorted array of tokens
	 */
	var sort = function(tokens) {
 ...
 * 
 * Patch #6:
 * One more additional argument that allows to ignore cache and avoid re-sorting while doing work.
 ...
 * (options.tolerant and options.noCache) -- see patches #5 and #6 notes
 */
tokenize = Sizzle.tokenize = function( selector, parseOnly, options) {
	...
	/** Patch #6: Sorting tokens */
	if (!noCache) {
		// We do the re-sorting hack only when tokenize is called internally.
		// In other cases let the tokens order be as it is in the original selector.
		var iGroups = groups.length;
		while (iGroups--) {
			groups[iGroups] = TokenSorter.sort(groups[iGroups]);
		}
	}
	...
	if (validLen !== 0) {
		...
		return {
			...
			tokens: (noCache ? groups : tokenCache(
				validSelector,
				groups
			)).slice(0),
			nextIndex: nextIndex
	...
	return (noCache ? groups : tokenCache( selector, groups )).slice( 0 );
};
