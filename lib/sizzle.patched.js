 ...
 * @param {} tokens tokens to post-process
 */
...
tokenize = Sizzle.tokenize = function( selector, parseOnly, options) {
	...
	var i = 0;
	for (; i < len; i++) {
		sortedGroups.push(TokenSorter.sort(groups[i]));
	}
	/** Patch #6: Change the way tokens are cached */
	var tokensCacheItem = {
		groups: groups,
	...
	if (validLen !== 0) {
		...
		var validSelector = selector.slice(0, nextIndex).trim();
		tokensCacheItem = tokenCache(validSelector, tokensCacheItem);

		return {
			selector: validSelector,
			tokens: (returnUnsorted ? tokensCacheItem.groups : tokensCacheItem.sortedGroups).slice(0),
			nextIndex: nextIndex
		};
	}
