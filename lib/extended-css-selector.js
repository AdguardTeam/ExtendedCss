...
const ExtendedSelectorFactory = (function () {
    ...
    UltimusSelector.prototype = {
        querySelectorAll() {
            ...
            simpleNodes.forEach((node) => {
                this.searchResultNodes(node, this.ultimusArg, resultNodes);
            });
         ...
         * @param {Array} result
         */
        searchResultNodes(node, ultimusArg, result) {
            if (ultimusArg) {
                result.push(node);
            }
        },
    ...
    XpathSelector.prototype.searchResultNodes = function (node, ultimusArg, result) {
        const xpathResult = document.evaluate(ultimusArg, node, null,
