...
const AttributesMatcher = (() => {
    ...
    const AttrMatcher = function (attrFilter, pseudoElement) {
        ...
        try {
            const index = attrFilter.indexOf('=');
            const inputAttrName = attrFilter.substring(0, index).trim();
            const inputAttrValue = attrFilter.substring(index + 1).trim();
    ...
    AttrMatcher.prototype.matches = function (element) {
        if (!this.attrNameRegexp || !this.attrValueRegexp) { return false; }

        let attrMatched = false;
        const elAttrs = element.attributes;
        if (elAttrs.length > 0) {
            for (let i = 0; i < elAttrs.length; i += 1) {
                const attr = elAttrs[i];
                attrMatched += this.attrNameRegexp.test(attr.name)
    ...
    const extendSizzle = function (sizzle) {
        ...
        sizzle.selectors.pseudos['attr-matches'] = sizzle.selectors.createPseudo((attrFilter) => {
