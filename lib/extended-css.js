...
function ExtendedCss(configuration) { // jshint ignore:line
    ...
    const EventTracker = (function() {
        ...
        return {
            // We are trying to limit the number of callback calls by not calling it on all kinds of "hover" events.
            ...
            shouldIgnoreMutations: function () {
                const lastEventType = getLastEventType();
                const timeSinceLastEvent = getTimeSinceLastEvent();
                return ignoredEventTypes.includes(lastEventType) && timeSinceLastEvent < LAST_EVENT_TIMEOUT_MS;
            }
        };
    })();
    ...
    function observeDocument(callback) {
        if (utils.MutationObserver) {
            domMutationObserver = new utils.MutationObserver(function(mutations) {
                ...
                if (EventTracker.shouldIgnoreMutations()) {
                    // call callback if there are mutations with types other than attributes
