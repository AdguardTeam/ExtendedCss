...
function ExtendedCss(configuration) { // jshint ignore:line
    ...
    function removeElement(affectedElement) {
        // keep current node element
        const node = affectedElement.node;

        // check if element already was removed early
        const removedElement = removedElements[utils.getNodeSelector(affectedElement.node)];
        if (removedElement) {
        ...
        affectedElement.nodeSelector = affectedElement.nodeSelector || utils.getNodeSelector(node);
