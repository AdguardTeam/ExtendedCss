...
/* global ExtendedCssParser, StyleObserver, utils */
 ...
 * @returns {Array|NodeList} a list of elements found
...
ExtendedCss.query = function(selectorText, noTiming) {
    ...
    try {
        // Adding an artifical style so that we could use ExtendedCssParser and 
        // normalize selector text just like ExtendedCss does when we use it
        let cssText = selectorText + ' { display: none; }';
        let rules = ExtendedCssParser.parseCss(cssText);

        if (!rules || rules.length !== 1) {
            throw 'Cannot parse ' + cssText;
        }

        return rules[0].selector.querySelectorAll();
    } finally {
        StyleObserver.clear();
