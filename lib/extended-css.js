    ...
    function observeDocument(callback) {
        if (utils.MutationObserver) {
            domMutationObserver = new utils.MutationObserver(function(mutations) {
                ...
                const lastEventType = EventTracker.getLastEventType();
                const timeSinceLastEvent = EventTracker.getTimeSinceLastEvent();
                if (ignoredEventTypes.includes(lastEventType) && timeSinceLastEvent < LAST_EVENT_TIMEOUT_MS) {
                    // ignore attributes mutations
                    const nonAttributeMutations = mutations.filter(function (mutation) {
                        return mutation.type !== 'attributes';
                    });
                    if (nonAttributeMutations.length > 0) {
                        callback();
                    }
                    return;
                }
