...
function ExtendedCss(configuration) { // jshint ignore:line
     ...
     */
    function createProtectionFunction(style) {
     ...
     */
    function protectStyleAttribute(node, style) {
        if (!utils.MutationObserver) { return null; }
        var protectionObserver = new utils.MutationObserver(createProtectionFunction(style));
    ...
    function applyStyle(affectedElement) {
        ...
        affectedElement.protectionObserver = protectStyleAttribute(node, style);
    }
