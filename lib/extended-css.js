...
function ExtendedCss(configuration) { // jshint ignore:line
    ...
    function findAffectedElement(node) {
        ...
        const removedElement = removedElements[getNodeSelector(node)];
        if (removedElement) {
            removedElement.node = node;
    ...
    function applyStyle(affectedElement) {
        ...
        if (style['remove'] === 'true') {
            // check if element already was removed early
            const removedElement = removedElements[getNodeSelector(affectedElement.node)];
            if (removedElement) {
                affectedElement = removedElement;
            }
            // if removals attempts happened more than specified we do not try to remove node again
            if (affectedElement.removalsCounter && affectedElement.removalsCounter > MAX_STYLE_PROTECTION_COUNT) {
                utils.logError('ExtendedCss: infinite loop protection for SELECTOR', affectedElement.nodeSelector);
                return;
            }
            affectedElement.nodeSelector = affectedElement.selector || getNodeSelector(node);
            affectedElement.removalsCounter = affectedElement.removalsCounter ? affectedElement.removalsCounter + 1 : 1;
            removedElements[affectedElement.nodeSelector] = affectedElement;
            if (node.parentNode) {
